...
// offline (nil Broadcaster) branches of broadcast/handleNext, batch.Delete's
...
// PubSubBroadcaster.Next's own-context-cancelled branch.
...
func TestPublishBroadcastError(t *testing.T) {
	...
	d, err := New(dssync.MutexWrap(ds.NewMapDatastore()), ds.NewKey("errbcast"), newTestDagsync(), &errBroadcaster{err: wantErr}, opts)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOfflineNilBroadcaster(t *testing.T) {
	opts := DefaultOptions()
	d, err := New(dssync.MutexWrap(ds.NewMapDatastore()), ds.NewKey("offline"), newTestDagsync(), nil, opts)
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _ = d.Close() }()
...
func TestCompactOfflineNilBroadcaster(t *testing.T) {
	opts := DefaultOptions()
	d, err := New(dssync.MutexWrap(ds.NewMapDatastore()), ds.NewKey("offline-compact"), newTestDagsync(), nil, opts)
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _ = d.Close() }()
...
// check but for batch.Delete: enough tombstones accumulated in one batch
...
func TestBatchDeleteTriggersImplicitCommit(t *testing.T) {
	...
	if !anyGoneAlready {
		t.Fatal("expected batch.Delete to have implicitly committed at least once before the explicit Commit()")
	}
...
// rather than hanging), so this needs a store built directly through New()
// to actually observe DAGSyncerTimeout == 0.
func TestWithDAGTimeoutDisabled(t *testing.T) {
	opts := DefaultOptions()
	opts.DAGSyncerTimeout = 0
	d, err := New(dssync.MutexWrap(ds.NewMapDatastore()), ds.NewKey("dagtimeout-disabled"), newTestDagsync(), nil, opts)
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _ = d.Close() }()
	...
	d2, err := New(dssync.MutexWrap(ds.NewMapDatastore()), ds.NewKey("dagtimeout-enabled"), newTestDagsync(), nil, opts2)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPubSubBroadcasterOwnContextDone(t *testing.T) {
	...
	bc, err := NewPubSubBroadcaster(ctx, ps, "own-ctx-done-topic")
	if err != nil {
		t.Fatal(err)
	}
