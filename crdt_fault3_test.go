...
// (reached via batch.Delete, distinct from Datastore.Delete's own call to
...
func TestBroadcastShortCircuits(t *testing.T) {
	...
	// time, rather than swapping store.broadcaster in after the fact: New()
	...
	opts := DefaultOptions()
	d, err := New(newFaultyDatastore(ds.NewMapDatastore()), ds.NewKey("broadcastshortcircuit"), newTestDagsync(), &errBroadcaster{err: errFault}, opts)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPutTombsErrors(t *testing.T) {
	t.Run("Batch fails", func(t *testing.T) {
		...
		d := newTestDatastore(t, fd)
		ctx := context.Background()
		k := ds.NewKey("puttombs-batch-key")
...
func TestDagWorkerProcessNodeError(t *testing.T) {
	...
	fd := newFaultyDatastore(ds.NewMapDatastore())
	d, err := New(fd, ds.NewKey("dagworkerfail"), newTestDagsync(), nil, opts)
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _ = d.Close() }()
