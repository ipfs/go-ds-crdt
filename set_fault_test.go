...
func newTestDatastore(t *testing.T, store ds.Datastore) *Datastore {
	t.Helper()
	opts := DefaultOptions()
	d, err := New(store, ds.NewKey("faulttest"), newTestDagsync(), nil, opts)
	if err != nil {
		t.Fatal(err)
	}
