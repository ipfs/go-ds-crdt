...
func TestCompactBroadcastError(t *testing.T) {
	...
	d, err := New(fd, ds.NewKey("compactbroadcastfail"), newTestDagsync(), bcast, opts)
	if err != nil {
		t.Fatal(err)
	}
...
func TestWalkProcessedDAGFetchError(t *testing.T) {
	...
	// New()'s background goroutines that read it concurrently.
	dagsvc := &erroringDAGService{DAGService: newTestDagsync()}
	opts := DefaultOptions()
	d, err := New(newFaultyDatastore(ds.NewMapDatastore()), ds.NewKey("walkfetchfail"), dagsvc, nil, opts)
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _ = d.Close() }()
