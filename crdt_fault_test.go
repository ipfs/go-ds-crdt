...
// Fault-injection coverage for crdt.go's New()/Close()/MarkDirty/IsDirty/
...
// TestNewBadShutdownKeyErrors exercises New()'s three bad-shutdown-key
...
func TestNewBadShutdownKeyErrors(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			opts := DefaultOptions()
			_, err := New(fd, ds.NewKey("newfail"), newTestDagsync(), nil, opts)
			if !errors.Is(err, errFault) {
				t.Fatalf("expected errFault from New() when %s, got %v", tc.name, err)
			}
		})
	}
}

// TestNewApplyMigrationsError checks that New() propagates a failure from
...
func TestNewApplyMigrationsError(t *testing.T) {
	...
	opts := DefaultOptions()
	_, err := New(fd, ds.NewKey("newmigfail"), newTestDagsync(), nil, opts)
	if !errors.Is(err, errFault) {
		t.Fatalf("expected errFault from New() propagating an applyMigrations failure, got %v", err)
	}
}
