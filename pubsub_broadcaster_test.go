...
// GossipSub instance, exercising PubSubBroadcaster end to end.
...
// PubSubBroadcaster is observed via Next() on a different, directly
...
func TestPubSubBroadcaster(t *testing.T) {
	...
	defer cancel1()
	bc1, err := NewPubSubBroadcaster(ctx1, ps1, topic)
	if err != nil {
		t.Fatal(err)
	}
	...
	defer cancel2()
	bc2, err := NewPubSubBroadcaster(ctx2, ps2, topic)
	if err != nil {
		t.Fatal(err)
	}
