...
func TestAddDAGNodePutBlockError(t *testing.T) {
	...
	// with New()'s background goroutines that read it concurrently):
	// set.Add below never touches the DAG service, so there is no need
	// for any prior successful write through it.
	opts := DefaultOptions()
	d, err := New(newFaultyDatastore(ds.NewMapDatastore()), ds.NewKey("adddagnodefail"), &erroringAddDAGService{DAGService: newTestDagsync()}, nil, opts)
	if err != nil {
		t.Fatal(err)
	}
...
// normal batch.Delete path once a batch already has a pending curDelta
